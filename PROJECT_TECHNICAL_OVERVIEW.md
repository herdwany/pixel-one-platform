...
│   └── supabase-client.js   → Singleton client, auth helpers, generateOrderRef()
├── database/
...
| `supabase-client.js` | `getSupabaseClient()`, `getCurrentUser()`, `getProfile(userId)`, `requireAuth(adminOnly?)`, `generateOrderRef()`, `buildWhatsAppLink(order)` |
...
| -------------------- | ------------- | --------------------------------------------------------------- |
| `id`                 | `bigint` PK   | Auto-identity                                                   |
| `order_ref`          | `text` UNIQUE | Generated client-side via `generateOrderRef()`                  |
...
| `is_approved`  | `boolean`       | Default `true` (auto-publish) — changed in 013                                   |
...
- Comments have `is_approved` defaulting to `true` (migration 013).
- This means comments appear immediately without admin moderation.
- Admin can still delete inappropriate comments from the Comments section.
...
| Order refs          | Generated client-side via `generateOrderRef()` → `PX-XXXXXX`         |
