...
| **Frontend**    | HTML5, Tailwind CSS (CDN + build pipeline), Vanilla JavaScript (ES6+)      |
...
| **Package Mgr** | npm — `supabase`, `tailwindcss`, `postcss`, `autoprefixer`                 |
...
│   ├── api/                 → Centralized data-access modules (services, orders, blog, users)
│   ├── admin/               → Admin section modules (orders/users/blog/comments/services/content)
│   ├── utils/               → Error handling + cache utilities
│   └── state/               → Shared lightweight frontend state
├── database/
│   ├── setup.sql            → Full initial schema
│   └── migrations/          → 001–014 incremental migrations
├── assets/css/
│   └── tailwind.input.css   → Tailwind source entry for minified build output
...
**RLS:** Users read/update own. Public insert is blocked; notifications are generated by DB triggers (reply/like/order status). Admin manages all.
